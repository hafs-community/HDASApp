help([[
Load environment for running the GDAS application with Intel compilers and MPI on Ursa
]])

prepend_path("MODULEPATH", "/contrib/spack-stack/spack-stack-1.9.1/envs/ue-oneapi-2024.2.1/install/modulefiles/Core")

load("stack-oneapi/2024.2.1")
load("stack-intel-oneapi-mpi/2021.13")
load("cmake/3.27.9")
load("gettext/0.22.5")
--load("pcre2/10.42")
load("curl/8.10.1")
load("zlib/1.2.13")
load("git/2.43.5")
--load("pkg-config/0.29.2")
load("hdf5/1.14.3")
load("parallel-netcdf/1.12.3")
load("netcdf-c/4.9.2")
load("nccmp/1.9.0.1")
load("netcdf-fortran/4.6.1")
load("nco/5.2.4")
load("parallelio/2.6.2")
load("wget/1.21.1")
load("boost/1.84.0")
load("bufr/12.1.0")
load("git-lfs/3.5.1")
load("ecbuild/3.7.2")
load("openjpeg/2.3.1")
load("eccodes/2.34.0")
load("eigen/3.4.0")
load("openblas/0.3.24")
load("eckit/1.28.3")
load("fftw/3.3.10")
load("fckit/0.13.2")
load("fiat/1.4.1")
load("ectrans/1.5.0")
load("fms/2024.02")
load("esmf/8.8.0")
load("atlas/0.40.0")
load("sp/2.5.0")
load("gsl-lite/0.37.0")
load("libjpeg/2.1.0")
load("krb5/1.21.1-1")
load("libtirpc/1.3.3")
load("hdf/4.2.15")
load("jedi-cmake/1.4.0")
load("libpng/1.6.37")
load("libxaw/1.0.16")
load("udunits/2.2.28")
load("ncview/2.1.9")
load("netcdf-cxx4/4.3.1")
load("json/3.11.3")
load("crtm/2.4.0.1")
load("rocoto/1.3.7")
load("prod_util/2.1.1")

load("py-jinja2/3.1.4")
load("py-netcdf4/1.7.1.post2")
load("py-pybind11/2.13.5")
load("py-pycodestyle/2.11.0")
load("py-pyyaml/6.0.2")
load("py-scipy/1.14.1")
load("py-xarray/2024.7.0")
load("py-f90nml/1.4.3")
load("py-pip/23.1.2")

setenv("CC", "mpiicx")
setenv("CXX", "mpiicpx")
setenv("FC", "mpiifort")
setenv("I_MPI_CC", "icx")
setenv("I_MPI_CXX", "icpx")
setenv("I_MPI_F90", "ifort")

local mpiexec = '/usr/bin/srun'
local mpinproc = '-n'
setenv('MPIEXEC_EXEC', mpiexec)
setenv('MPIEXEC_NPROC', mpinproc)

setenv("CRTM_FIX","/scratch3/HFIP/hwrfv3/GDASApp/fix/crtm/2.4.0")
setenv("GDASAPP_TESTDATA","/scratch3/HFIP/hwrfv3/GDASApp/testdata")
setenv("GDASAPP_UNIT_TEST_DATA_PATH","/scratch3/HFIP/hwrfv3/GDASApp/unittestdata")


whatis("Description: Load all libraries needed for GDASApp")
